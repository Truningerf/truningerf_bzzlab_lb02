...
function checkEmail(id, input){
    ...
    if (!re.test(input.trim())) {
        result = {
            isNotValid: true,
            msg: showError(id, 'Email is not valid. Use the form samlpe@example.xx')
        }
    }
    return result;
}
...
function checkPhone(id, input){
    ...
    if (re.test(input.trim())) {
        result = {
            isNotValid: true,
            msg: showError(id, 'Phone number is not valid. Use the form +41 11 111 11 11 or 011 111 11 11')
        }
    }
    return result;
}
...
function checkDate(id, input){
    ...
    if (re.test(input.trim())) {
        result = {
            isNotValid: true,
            msg: showError(id, 'Date is not valid. Use the form dd/mm/yy.')
        }
    }
    return result;
}
